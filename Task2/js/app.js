...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for(let t = 0; t < $(modalButtons).length; t++){
        $(modalButtons[t]).on('click', function(){
            ...
            if(!$(modalWindow).hasClass('modal-window-active')){
                $(modalWindow).addClass('modal-window-active')
                $(modalWindow).fadeIn()
            }
            else{
                $(modalWindow).fadeOut()
                $(modalWindow).removeClass('modal-window-active')
            }
        })
    }
